...
export function CompensationForm({ applicationId, userId, onSave, onError }) {
  ...
  useEffect(() => {
    function handleEscape(event) {
      if (event.key === "Escape" && isEditing && status !== "saving") {
        setIsEditing(false);
      }
    }
  ...
  function openModal() {
    if (compensation) {
      setFormData({
        ...
        bonusSalary: compensation.bonusSalary ?? "",
        signingBonus: compensation.signingBonus ?? "",
        stockEquity: compensation.stockEquity ?? "",
        benefits: compensation.benefits ?? "",
        currency: compensation.currency ?? "USD",
        payCadence: compensation.payCadence ?? "annual",
        locationType: compensation.locationType ?? "on-site",
  ...
  return (
    <>
      ...
      {isEditing && (
        <div
          className="compensation-modal-scrim"
          role="presentation"
          onClick={() => status !== "saving" && setIsEditing(false)}
        >
          <div
            className="compensation-modal"
            role="dialog"
            aria-modal="true"
            aria-label="Compensation form"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="panel-header">
              <h3>{compensation ? "Edit Compensation" : "Add Compensation"}</h3>
              <button
                type="button"
                className="btn btn-subtle"
